<?php
/**
  * @var \App\View\AppView $this
  */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Youtube Url'), ['action' => 'edit', $youtubeUrl->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Youtube Url'), ['action' => 'delete', $youtubeUrl->id], ['confirm' => __('Are you sure you want to delete # {0}?', $youtubeUrl->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Youtube Urls'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Youtube Url'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Users'), ['controller' => 'Users', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New User'), ['controller' => 'Users', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="youtubeUrls view large-9 medium-8 columns content">
    <h3><?= h($youtubeUrl->title) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Title') ?></th>
            <td><?= h($youtubeUrl->title) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Url') ?></th>
            <td><?= h($youtubeUrl->url) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('User') ?></th>
            <td><?= $youtubeUrl->has('user') ? $this->Html->link($youtubeUrl->user->id, ['controller' => 'Users', 'action' => 'view', $youtubeUrl->user->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($youtubeUrl->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Youtube Category') ?></th>
            <td><?= $this->Number->format($youtubeUrl->youtube_category) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Created') ?></th>
            <td><?= h($youtubeUrl->created) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Modified') ?></th>
            <td><?= h($youtubeUrl->modified) ?></td>
        </tr>
    </table>
</div>
